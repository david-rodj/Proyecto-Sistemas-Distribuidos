...
public class HealthCheckManager {
    ...
    public void start() {
        ...
        System.out.println("ðŸš€ Iniciando HealthCheck Manager AsÃ­ncrono...");
        ...
        try (ZContext context = new ZContext()) {
            ...
            System.out.println("ðŸ“¡ Frontend ROUTER escuchando en puerto " + DEPARTMENT_PORT);



            // Socket para comunicaciÃ³n con servidores backend
            ...
            System.out.println("âœ… Sistema listo para procesar requests...");
            ...
            System.err.println("âŒ Error en HealthCheck Manager: " + e.getMessage());
    ...
    private void handleIncomingRequest(ZMQ.Socket frontend, ZMQ.Socket backend) {
        ...
        try {
            ...
            if (parts.length < 1) {
                ...
                sendErrorResponse(frontend, clientIdentity, "Formato de mensaje invÃ¡lido");
            ...
            // Crear contexto para tracking asÃ­ncrono
            ...
            // Enviar al servidor backend de forma asÃ­ncrona
            ...
            requestProcessor.submit(() -> {
                ...
                try {
                    ...
                    System.err.println("âŒ Error enviando request " + requestId + ": " + e.getMessage());
            ...
            System.err.println("âŒ Error procesando request entrante: " + e.getMessage());
    ...
    private void handleBackendResponse(ZMQ.Socket backend, ZMQ.Socket frontend) {
        ...
        try {
            ...
            if (context != null) {
                ...
                System.err.println("âš ï¸ Recibida respuesta para request desconocido: " + requestId);
            ...
            System.err.println("âŒ Error procesando respuesta del backend: " + e.getMessage());
            ...
            checkServerHealth(context, PRIMARY_HEALTH, "PRIMARY", 
                ...
                () -> {
                    ...
                    synchronized (failureCountLock) {
                        ...
                        System.out.println("âœ… Servidor PRIMARY recuperado");
                ...
                () -> {
                    ...
                    synchronized (failureCountLock) {
                        ...
                        if (primaryFailureCount >= CIRCUIT_BREAKER_THRESHOLD) {
                            ...
                            System.err.println("ðŸš¨ Servidor PRIMARY marcado como inactivo");
            ...
            checkServerHealth(context, BACKUP_HEALTH, "BACKUP",
                ...
                () -> {
                    ...
                    synchronized (failureCountLock) {
                        ...
                        System.out.println("âœ… Servidor BACKUP recuperado");
                ...
                () -> {
                    ...
                    synchronized (failureCountLock) {
                        ...
                        if (backupFailureCount >= CIRCUIT_BREAKER_THRESHOLD) {
                            ...
                            System.err.println("ðŸš¨ Servidor BACKUP marcado como inactivo");
    ...
    throw new Exception("Send failed");
                ...
                System.err.println("âš ï¸ " + serverName + " - Respuesta inesperada: " + reply);
            ...
            System.err.println("âŒ " + serverName + " health check fallÃ³: " + e.getMessage());
    ...
    private void connectToActiveServer(ZMQ.Socket backend) {
        ...
        if (primaryServerActive.get()) {
            ...
            System.out.println("ðŸ”— Conectado al servidor PRIMARY");

        } else if (backupServerActive.get()) {

            backend.connect(BACKUP_SERVER);

            System.out.println("ðŸ”— Conectado al servidor BACKUP");

        } else {

            System.err.println("ðŸš¨ NO HAY SERVIDORES ACTIVOS");
        ...
        pendingRequests.entrySet().removeIf(entry -> {
            ...
            if (context.isExpired(TIMEOUT_MS * 2)) {
                ...
                try (ZContext tempContext = new ZContext()) {
                    ...
                    // Simular respuesta de timeout (en implementaciÃ³n real, 

                    // necesitarÃ­amos mantener referencia al frontend socket)
    ...
    private void startRequestCleanup() {
        ...
        healthCheckScheduler.scheduleWithFixedDelay(() -> {
            ...
            if (cleaned > 0) {
                ...
                System.out.println("ðŸ§¹ Limpiados " + cleaned + " requests expirados");
    ...
    private void logRequest(String requestId, String message, String server) {
        ...
        System.out.println(String.format("ðŸ“¤ [%s] Request %s -> %s: %s", 
    ...
    private void logResponse(String requestId, String response) {
        ...
        System.out.println(String.format("ðŸ“¥ [%s] Response %s: %s", 
    ...
    private void shutdown() {
        ...
        System.out.println("ðŸ›‘ Cerrando HealthCheck Manager...");
        ...
        System.out.println("âœ… Shutdown completado");
